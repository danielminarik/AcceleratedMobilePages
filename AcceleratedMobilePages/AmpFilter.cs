...
namespace AcceleratedMobilePages
{
    class AmpFilter
    {
        ...
        {
            int state = AcceleratedMobilePages.CheckStateHelper.CheckFilterState();
        ...
        {
            string ampLink = CMS.Helpers.CMSHttpContext.Current.Request.IsSecureConnection ? Constants.P_HTTPS : Constants.P_HTTP + 
        ...
        {
            // Initial html amp tag - replace only first occurence
        ...
        private string InsertCompulsoryMarkupAndCss(string finalHtml)
        {
            ...
            string canonicalLink = CMS.Helpers.CMSHttpContext.Current.Request.IsSecureConnection ? Constants.P_HTTPS : Constants.P_HTTP +
        ...
        private string GetStylesheetText()
        {
            ...
            if (q.FirstObject.UseDefaultStylesheet)
            {
                ...
                {
                    var cssInfo = CMS.PortalEngine.CssStylesheetInfoProvider.GetCssStylesheetInfo(cssID);
                    if (cssInfo != null)
                    {
            ...
            {
                var cssInfo = CMS.PortalEngine.CssStylesheetInfoProvider.GetCssStylesheetInfo(q.FirstObject.StylesheetID);
                if (cssInfo != null)
                {
                    cssText = cssInfo.StylesheetText;
                }
            ...
            // Resolve client URL
            return HTMLHelper.ResolveCSSClientUrls(cssText, CMS.Helpers.CMSHttpContext.Current.Request.Url.ToString());
        }
